...
// Find the biggest step in x
  dx=xy(2:$,1)-xy(1:$-1,1)
  [val,ipos]=max(abs(dx))
// If the step > 75% * image width, move the step to start of the array
  if( val > ImageInfo.numxy(1)*0.75) then
...
// All 3 lines are dashed and yellow
  MainLines=[]
  for i=1:3
    xpoly(0,0,"lines")
    MainLines(i)=gce()
